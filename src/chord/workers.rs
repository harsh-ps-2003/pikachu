...
async fn stabilize(config: &ThreadConfig) -> Result<(), ChordError> {
    ...
    let successor_addr = config.get_node_addr(&successor).await
        .ok_or_else(|| ChordError::NodeNotFound(format!("Address not found for successor {}", successor)))?;
        
    // Create client for successor
    let mut successor_client = ChordGrpcClient::new(successor_addr.clone())
        .await
        .map_err(|e| ChordError::StabilizationFailed(format!("Failed to connect to successor: {}", e)))?;
    ...
    let successor_pred = match successor_client.get_predecessor().await {
        ...
        Err(e) => return Err(ChordError::StabilizationFailed(format!("Failed to get predecessor from successor: {}", e))),
    };
    ...
    if let Some(x) = successor_pred {
        if x != config.local_node_id && x.is_between(&config.local_node_id, &successor) {
            ...
            info!("Updated successor from {} to {} during stabilization", 
                  successor, x);
    ...
    successor_client.notify(notify_request)
        .await
        .map_err(|e| ChordError::StabilizationFailed(format!("Failed to notify successor: {}", e)))?;
            ...
            match client.get_successor_list().await {
                Ok(list) => {
                    ...
                    for node_info in list {
                        ...
                        if !successor_list.contains(&node) && successor_list.len() < MAX_SUCCESSOR_LIST_SIZE {
...
async fn attempt_emergency_recovery(config: &ThreadConfig) -> Result<(), ChordError> {
    ...
    Err(ChordError::StabilizationFailed("No live nodes found for recovery".into()))
}
...
async fn find_successor_for_finger(config: &ThreadConfig, id: &NodeId) -> Result<NodeId, ChordError> {
    ...
    if let Some(node) = closest {
        if node == config.local_node_id {
            ...
            let finger_table = config.finger_table.lock().await;
            return finger_table.get_successor()
                .ok_or_else(|| ChordError::FingerUpdateFailed("No successor found".to_string()));
        }
        
        // Get the node's address and create client
        let addr = config.get_node_addr(&node).await
            .ok_or_else(|| ChordError::NodeNotFound(format!("Address not found for node {}", node)))?;
            
        let mut client = ChordGrpcClient::new(addr)
            .await
            .map_err(|e| ChordError::FingerUpdateFailed(format!("Failed to connect to node: {}", e)))?;
        ...
        match client.find_successor(id.to_bytes().to_vec()).await {
            ...
            Err(e) => Err(ChordError::FingerUpdateFailed(format!("Failed to find successor: {}", e)))
        }
    } else {
